...
int main(int argc, char **argv)
{
    ...
    shm_initsend(shm, rem_shmid);

    if (rem_shmid < shm->recv_id) {
	int i;
	int len;
	...
	for (i = 1; i <= 20; i++) {
	    len = sprintf(buf, "Hallo %d", i);
	    shm_send(shm, buf, len);
	}
	shm_send(shm, buf, BUFSIZE);
	shm_send(shm, NULL, 0);
...
/*
 * Local Variables:
 *  compile-command: "gcc shmpp.c -g -Wall -W -Wno-unused -O2 -fverbose-asm -save-temps -o shmpp"
 * End:
 *
 */
