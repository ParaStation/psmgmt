...
/* binary tree */
struct t_node *insertTNode(PStask_ID_t key, Job_t *job, struct t_node *leaf);
...
void timer_handler(int signum)
{
    ...
    else
    {
       ...
       char chead[300];
       sprintf(chead,"%s;%s;%i.%s",ctime,"E",PSC_getPID(job->logger),job->hostname);
...
void printAccEndMsg(char *chead, PStask_ID_t key)
{
     ...
     if(rss)
	 sprintf(used_mem,"resources_used.mem=%ldkb ", rss);
...
void handleAccQueueMsg(char *chead, char *ptr, PStask_ID_t logger)
{
    ...
    job->countSlotMsg = 0;
    job->incomplete = 0;
    job->exec_hosts = malloc(EXEC_HOST_SIZE);
...
void handleAccEndMsg(char *chead, char *ptr, PStask_ID_t sender, PStask_ID_t logger) 
{
    ...
    if(!job){
	  ...
	  printAccEndMsg(chead, logger);
	   ...
	   job->countExitMsg++;
    	   job->cput += job->rusage.ru_utime.tv_sec + 1.0e-6 * job->rusage.ru_utime.tv_usec + job->rusage.ru_stime.tv_sec + 1.0e-6 * job->rusage.ru_stime.tv_usec;
...
void openAccLogFile(char *arg_logdir)
{
	...
	if((arg_logdir && fp && !strcmp(arg_logdir,"-") ) || (fp && !strcmp(filename,oldfilename))){
		return;
	}
	...
	if(fp && !!strcmp(filename,oldfilename)){
		...
		if(logPostProcessing){
			char syscmd[1024];
			sprintf(syscmd,"%s ",logPostProcessing);
			if(arg_logdir)
				strcat(syscmd,arg_logdir);
			else
				strcat(syscmd,DEFAULT_LOG_DIR);
			strcat(syscmd,"/");
			strcat(syscmd,oldfilename);
			system(syscmd);					
		}
	...
	if(arg_logdir){
   		if(!strcmp(arg_logdir,"-"))
			fp = stdout; //fdopen(stdout);
	 		...
	 		sprintf(alogfile,"%s/%s",arg_logdir,filename);
			fp = fopen(alogfile,"a+");
		}
	}
	else{
	 	sprintf(alogfile,"%s/%s",DEFAULT_LOG_DIR,filename);
		fp = fopen(alogfile,"a+");
	}
