...
/***************************************************************************/
// Stuff stolen from MPL
static int MPL_strncpy(char *dest, const char *src, size_t n)
{
...
static int MPL_strnapp(char *dest, const char *src, size_t n)
{
    char *restrict d_ptr = dest;
    const char *restrict s_ptr = src;
    register int i;
...
static int PMIi_InitIfSingleton(void)
{
    if (PMI_initialized != NORMAL_INIT_WITH_PM) return PMI_FAIL;
    return PMI_SUCCESS;
}
