...
for p in helper.partitions():
	...
	for suffix in ["out", "err"]:
		for i in range(4):
			fn = "%s/slurm-%s-%s.%s" % (os.environ["PSTEST_OUTDIR"], \
			                            os.environ["PSTEST_SCONTROL_%s_JOB_ID" % p.upper()], hosts[i], suffix)
			...
			try:
				...
				test.check("%d %s" % (2*i    , hosts[i]) == lines[0], p)
				test.check("%d %s" % (2*i + 1, hosts[i]) == lines[1], p)
