...
for p in helper.partitions():
	...
	try:
		for i in range(3):
			...
			# Fix a Slurm bug
			err = re.sub(r'%8A', "%08d" % int(jobid) , err)
			err = re.sub(r'%4a', "%04d" % int(taskid), err)
			out = re.sub(r'%8A', "%08d" % int(jobid) , out)
			out = re.sub(r'%4a', "%04d" % int(taskid), out)

			test.check(os.path.isfile(out), p)
			test.check(os.path.isfile(err), p)

			test.check(re.match(r'.*slurm-%08d-x-%04d.out' % \
			                    (int(os.environ["PSTEST_SCONTROL_%s_%d_ARRAY_JOB_ID" % (P, i)]), int(taskid)), out), p)
			test.check(re.match(r'.*slurm-%08d-x-%04d.err' % \
			                    (int(os.environ["PSTEST_SCONTROL_%s_%d_ARRAY_JOB_ID" % (P, i)]), int(taskid)), err), p)
			...
			out = "%s/slurm-%08d-z-%04d.out" % (os.environ["PSTEST_OUTDIR"], int(jobid), int(taskid))
			err = "%s/slurm-%08d-z-%04d.err" % (os.environ["PSTEST_OUTDIR"], int(jobid), int(taskid))
