...
{
    uint32_t max = stat->jobInfoCount + Step_count();
    stat->jobInfo = urealloc(stat->jobInfo, sizeof(*stat->jobInfo) * max);
	...
	Slurm_Step_Head_t *head = &(stat->jobInfo)[stat->jobInfoCount];
