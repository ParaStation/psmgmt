...
int validate_perftool_list()
{
	...
	while (token) {
		++n;
		for (i = 0; _known_perftools[i]; ++i) {
			if (!strcmp(_known_perftools[i], token)) {
				++k;
			}
		}
		token = strtok(NULL, ",");
...
static struct spank_option opts[] = {
	{"perf-tool", "csl", "Enable performance analysis tools on nodes (experimental)", 1, 0, spank_option_cb},
...
int slurm_spank_init_post_opt(spank_t sp, int ac, char **av)
{
	...
	if (_perftool_list){
		...
		spank_job_control_setenv(sp, "SLURM_SPANK_PERF_TOOL_LIST", _perftool_list, 1);
	}
	else{
