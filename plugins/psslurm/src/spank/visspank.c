...
static int spank_option_cb(int val, const char *optarg, int remote)
{
	...
	ok = 0;
	for (i = 0; _supported[i]; ++i) {
		if (!strcmp(_supported[i], _start_xserver)) {
			ok = 1;
		}
	}

	if (!ok) {
		slurm_error("Invalid argument %s to option --start-xserver", _start_xserver);
		return -1;
	}
...
static struct spank_option opts[] = {
	{"start-xserver", NULL, "Start an Xserver for usage with VirtualGL", 2, 0, spank_option_cb},
...
int slurm_spank_init_post_opt(spank_t sp, int ac, char **av)
{
	if (_start_xserver) {
		...
		spank_job_control_setenv(sp, "SLURM_SPANK_START_XSERVER", _start_xserver, 1);
	}
