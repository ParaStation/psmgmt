...
const ConfDef_t confDef[] =
{
    ...
    { "ENERGY_SCRIPT", false, "file", "monitor_energy.sh",
    ...
    { "FILESYSTEM_SCRIPT", false, "file", "monitor_filesystem.sh",
...
bool initPSAccConfig(char *cfgName)
{
	...
	flog("failed to open '%s'\n", cfgName);
	return false;
    }
    if (traverseConfig(config, verifyVisitor, confDef)) return false;
